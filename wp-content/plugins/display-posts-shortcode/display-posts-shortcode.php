...
function be_display_posts_shortcode( $atts ) {
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $include_title && $include_link ) {
			...
			$title = '<a class="card-title" href="' . apply_filters( 'the_permalink', get_permalink() ) . '">' . get_the_title() . '</a>';

		} elseif ( $include_title ) {
			$title = '<span class="card-title">' . get_the_title() . '</span>';
		...
		if ( $image_size && has_post_thumbnail() && $include_link ) {
			$image = '<div class="card-img-top"><a class="image" href="' . get_permalink() . '">' . get_the_post_thumbnail( get_the_ID(), $image_size ) . '</a></div> ';

		} elseif ( $image_size && has_post_thumbnail() ) {
			$image = '<span class="image card-img-top">' . get_the_post_thumbnail( get_the_ID(), $image_size ) . '</span> ';
		...
		if ( $include_excerpt ) {
			...
			if ( $excerpt_length || $excerpt_more || $excerpt_more_link ) {
				...
				$length = $excerpt_length ? $excerpt_length : apply_filters( 'excerpt_length', 35 );
		...
		$output = '<div class="col-md-6 col-lg-4"><' . $inner_wrapper . ' class="' . implode( ' ', $class ) . '">' . $image . $title . $date . $author . $category_display_text . $excerpt . $content . '</' . $inner_wrapper . '></div>';
